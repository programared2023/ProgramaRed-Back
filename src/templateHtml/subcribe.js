module.exports = {
	mail: function (username) {
		let correoPersonalizado = `<body>
	<div class="es-wrapper-color">
		<!--[if gte mso 9]>
				<v:background xmlns:v="urn:schemas-microsoft-com:vml" fill="t">
					<v:fill type="tile" color="#aad576"></v:fill>
				</v:background>
			<![endif]-->
		<table class="es-wrapper" width="100%" cellspacing="0" cellpadding="0">
			<tbody>
				<tr>
					<td class="esd-email-paddings" valign="top">
						<table cellpadding="0" cellspacing="0" class="esd-header-popover es-header" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table bgcolor="#ffffff" class="es-header-body" align="center" cellpadding="0" cellspacing="0" width="580">
											<tbody>
												<tr>
													<td class="esd-structure esdev-adapt-off" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="580" class="es-m-p0r esd-container-frame" valign="top" align="center">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://front-programa-red.vercel.app/home"><img class="adapt-img" src="https://gtfqov.stripocdn.email/content/guids/CABINET_b2f67bc50193621608307ddd687d92fa78e9fe1ea683a669f30bfc371a263439/images/logonombre.png" alt style="display: block;" width="205"></a></td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
						<table class="es-content" cellspacing="0" cellpadding="0" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table class="es-content-body" style="background-color: #ffffff;" width="580" cellspacing="0" cellpadding="0" bgcolor="#ffffff" align="center">
											<tbody>
												<tr>
													<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="500" class="esd-container-frame" align="center" valign="top">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-text">
																						<p style="font-size: 31px; font-family: 'courier new', courier, 'lucida sans typewriter', 'lucida typewriter', monospace;">🚀¡¡¡Gracias por tu Suscripción!!!😀</p>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
												<tr>
													<td class="esd-structure es-p10t es-p10b es-p40r es-p40l" align="left">
														<table width="100%" cellspacing="0" cellpadding="0">
															<tbody>
																<tr>
																	<td class="es-m-p0r esd-container-frame" width="500" valign="top" align="center">
																		<table width="100%" cellspacing="0" cellpadding="0">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-image es-p40r es-p40l es-m-p0r es-m-p0l" style="font-size: 0px;"><a target="_blank" href="https://front-programa-red.vercel.app/home"><img class="adapt-img" src="https://i.pinimg.com/originals/1a/08/90/1a0890fe128b67f16ee259243e7d8557.jpg" alt="Happy St. Patrick Day" style="display: block;" title="Happy St. Patrick Day" height="570"></a></td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
						<table cellpadding="0" cellspacing="0" class="es-content" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
											<tbody>
												<tr>
													<td class="esd-structure es-p40" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="500" class="esd-container-frame" align="center" valign="top">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-text es-p10b es-p40r es-p40l es-m-p0r es-m-p0l">
																						<h1 style="font-size: 32px; text-align: left;">Querido/a ${username}</h1>
																					</td>
																				</tr>
																				<tr>
																					<td align="left" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0">
																						<p style="font-size: 16px;">¡Gracias por suscribirte a ProgramaRed Premium! Nos emociona mucho tenerte en nuestra comunidad de programadores y estamos ansiosos por compartir contigo nuestros conocimientos, recursos y noticias relacionadas con la programación.</p>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
						<table cellpadding="0" cellspacing="0" class="es-content" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
											<tbody>
												<tr>
													<td class="esd-structure es-p40t es-p40b es-p40r es-m-p40l" align="left" esdev-config="h2">
														<!--[if mso]><table width="540" cellpadding="0" cellspacing="0"><tr><td width="234" valign="top"><![endif]-->
														<table cellpadding="0" cellspacing="0" class="es-left" align="left">
															<tbody>
																<tr>
																	<td width="234" class="es-m-p20b esd-container-frame" align="left">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-image es-p40t es-p40b es-m-p0" style="font-size: 0px;"><a target="_blank" href><img class="adapt-img b_image" src="https://i.pinimg.com/564x/d2/34/99/d23499e438796b01dd7c1ebe28eb1c74.jpg" alt style="display: block;" width="234"></a></td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
														<!--[if mso]></td><td width="30"></td><td width="276" valign="top"><![endif]-->
														<table cellpadding="0" cellspacing="0" class="es-right" align="right">
															<tbody>
																<tr>
																	<td width="276" align="left" class="esd-container-frame">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="left" class="esd-block-text es-p20b es-p40r es-m-p0r">
																						<h2 class="b_title" style="font-size: 22px;">Nos esforzamos por crear un espacio inclusivo y acogedor para <strong>todos los niveles de habilidad</strong>. Ya seas un/a principiante o un/a programador/a experimentado/a, estamos aquí para apoyarte en tu camino hacia el <strong>éxito</strong> en el <strong>mundo de la programación</strong>.</h2>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
														<!--[if mso]></td></tr></table><![endif]-->
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
						<table cellpadding="0" cellspacing="0" class="es-content" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table bgcolor="#edfbbb" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580" style="background-color: #edfbbb;">
											<tbody>
												<tr>
													<td class="esd-structure es-p40" align="left" esdev-config="h3">
														<!--[if mso]><table dir="ltr" cellpadding="0" cellspacing="0"><tr><td><table dir="rtl" width="500" cellpadding="0" cellspacing="0"><tr><td dir="ltr" width="204" valign="top"><![endif]-->
														<table cellpadding="0" cellspacing="0" class="es-right" align="right">
															<tbody>
																<tr>
																	<td width="204" align="left" class="esd-container-frame es-m-p20b">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-image es-p40t es-p40b es-p40r es-m-p0" style="font-size: 0px;"><a target="_blank" href><img class="adapt-img b_image" src="https://i.pinimg.com/564x/9c/9c/a2/9c9ca2f6162b404fbbb813f194a56e40.jpg" alt style="display: block;" width="164"></a></td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
														<!--[if mso]></td><td dir="ltr" width="30"></td><td dir="ltr" width="266" valign="top"><![endif]-->
														<table cellpadding="0" cellspacing="0" class="es-left" align="left">
															<tbody>
																<tr>
																	<td width="266" align="left" class="esd-container-frame">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="left" class="esd-block-text es-p20b es-p40l es-m-p0l">
																						<h2 class="b_title" style="font-size: 24px;"><strong>La membresía Premium</strong> te permitirá compartir <strong>imágenes,&nbsp; &nbsp; videos y otros archivos con programadores</strong> y ampliar tu conocimiento con la experiencia de otros <strong>miembros</strong> de la comunidad de programación.</h2>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
														<!--[if mso]></td></tr></table></td></tr></table><![endif]-->
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
						<table cellpadding="0" cellspacing="0" class="es-content" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
											<tbody>
												<tr>
													<td class="esd-structure es-p40t es-p40r es-p40l" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="500" class="esd-container-frame" align="center" valign="top">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-text es-p20t es-m-txt-c">
																						<h2><br></h2>
																					</td>
																				</tr>
																				<tr>
																					<td align="center" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0r es-m-p0l">
																						<p>Si tienes alguna pregunta o sugerencia, no dudes en ponerte en contacto con nosotros. Estamos aquí para ayudarte en todo lo que necesites.<br></p>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
												<tr>
													<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
														<!--[if mso]><table width="500" cellpadding="0" cellspacing="0"><tr><td width="240" valign="top"><![endif]-->
														<table cellpadding="0" cellspacing="0" class="es-left" align="left">
															<tbody>
																<tr>
																	<td width="240" class="es-m-p20b esd-container-frame" align="left">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-button">
																						<!--[if mso]><a href="https://front-programa-red.vercel.app/" target="_blank" hidden>
		<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" esdevVmlButton href="https://front-programa-red.vercel.app/" 
					style="height:48px; v-text-anchor:middle; width:117px" arcsize="0%" stroke="f"  fillcolor="#538d22">
			<w:anchorlock></w:anchorlock>
			<center style='color:#ffffff; font-family:Sora, Arial, sans-serif; font-size:19px; font-weight:700; line-height:19px;  mso-text-raise:1px'>HOME</center>
		</v:roundrect></a>
	<![endif]-->
																						<!--[if !mso]><!-- --><span class="msohide es-button-border"><a href="https://front-programa-red.vercel.app/" class="es-button" target="_blank" style="font-weight: bold; font-size: 24px;">HOME</a></span>
																						<!--<![endif]-->
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
														<!--[if mso]></td><td width="20"></td><td width="240" valign="top"><![endif]-->
														<table cellpadding="0" cellspacing="0" class="es-right" align="right">
															<tbody>
																<tr>
																	<td width="240" align="left" class="esd-container-frame">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="https://front-programa-red.vercel.app/about" target="_blank" hidden>
		<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" esdevVmlButton href="https://front-programa-red.vercel.app/about" 
					style="height:48px; v-text-anchor:middle; width:129px" arcsize="0%" stroke="f"  fillcolor="#538d22">
			<w:anchorlock></w:anchorlock>
			<center style='color:#ffffff; font-family:Sora, Arial, sans-serif; font-size:19px; font-weight:700; line-height:19px;  mso-text-raise:1px'>ABOUT</center>
		</v:roundrect></a>
	<![endif]-->
																						<!--[if !mso]><!-- --><span class="msohide es-button-border"><a href="https://front-programa-red.vercel.app/about" class="es-button" target="_blank" style="font-size: 24px; font-weight: bold;">ABOUT</a></span>
																						<!--<![endif]-->
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
														<!--[if mso]></td></tr></table><![endif]-->
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
						<table cellpadding="0" cellspacing="0" class="esd-footer-popover es-footer" align="center">
							<tbody>
								<tr>
									<td class="esd-stripe" align="center">
										<table bgcolor="#ffffff" class="es-footer-body" align="center" cellpadding="0" cellspacing="0" width="580">
											<tbody>
												<tr>
													<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="500" class="esd-container-frame" align="center" valign="top">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-text">
																						<p><u>podes Comunicate con Nostros</u></p>
																						<p>programared2023@gmail.com</p>
																						<p>o contactar con nosotros en&nbsp;nuestro gitHub</p>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
												<tr>
													<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="500" class="esd-container-frame" align="center" valign="top">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://github.com/programared2023"><img class="adapt-img" src="https://icones.pro/wp-content/uploads/2021/06/icone-github-verte.png" alt style="display: block;" width="80"></a></td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
												<tr>
													<td class="esd-structure es-p20t es-p40b es-p40r es-p40l" align="left">
														<table cellpadding="0" cellspacing="0" width="100%">
															<tbody>
																<tr>
																	<td width="500" class="esd-container-frame" align="left">
																		<table cellpadding="0" cellspacing="0" width="100%">
																			<tbody>
																				<tr>
																					<td align="center" class="esd-block-spacer es-p30t es-p30b es-p40r es-p40l es-m-p0r es-m-p0l" style="font-size:0">
																						<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
																							<tbody>
																								<tr>
																									<td style="border-bottom: 2px solid #538d22; background: unset; height: 1px; width: 100%; margin: 0px;"></td>
																								</tr>
																							</tbody>
																						</table>
																					</td>
																				</tr>
																			</tbody>
																		</table>
																	</td>
																</tr>
															</tbody>
														</table>
													</td>
												</tr>
											</tbody>
										</table>
									</td>
								</tr>
							</tbody>
						</table>
					</td>
				</tr>
			</tbody>
		</table>
	</div>
	</body>
	`
		return correoPersonalizado;
	},
	mailRegister: function (username) {
		let bienvenida = `
			<body>
				<div class="es-wrapper-color">
					<!--[if gte mso 9]>
						<v:background xmlns:v="urn:schemas-microsoft-com:vml" fill="t">
							<v:fill type="tile" color="#aad576"></v:fill>
						</v:background>
					<![endif]-->
					<table class="es-wrapper" width="100%" cellspacing="0" cellpadding="0">
						<tbody>
							<tr>
								<td class="esd-email-paddings" valign="top">
									<table cellpadding="0" cellspacing="0" class="esd-header-popover es-header" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-header-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure esdev-adapt-off" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="580" class="es-m-p0r esd-container-frame" valign="top" align="center">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://front-programa-red.vercel.app/home"><img class="adapt-img" src="https://gtfqov.stripocdn.email/content/guids/CABINET_bc226981bf9a9de9b3e1f896df4c90639fbca5d662d15ccbfabf150976647ae8/images/logonombre.png" alt style="display: block;" width="205"></a></td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="es-content" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p40" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text es-p10b es-p40r es-p40l es-m-p0r es-m-p0l">
																									<h1 style="font-size: 32px; text-align: center;">¡Bienvenido a <span style="font-size:34px;">ProgramaRed&nbsp; ${username}😊</span>!&nbsp;</h1>
																								</td>
																							</tr>
																							<tr>
																								<td align="left" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0">
																									<p style="font-size: 16px;">👏Nos complace darle la bienvenida a nuestra Red Social 🙌, donde encontrará todo lo que necesita para aprender a programar y mejorar sus habilidades en el mundo digital 🌐. Aquí encontrará una amplia variedad de recursos , desde cursos 🎦, hasta herramientas útiles 🔧y artículos informativos para mantenerse al día con las últimas tendencias en tecnología.</p>
																									<p style="font-size: 16px;"><br></p>
																									<p>Únete a nuestra comunidad vibrante de programadores👥 apasionados y comienza tu viaje de aprendizaje con nosotros.</p>
																									<p><br></p>
																									<p style="font-size: 32px; text-align: center;">¡Bienvenido a bordo de ProgramaRed!🚀</p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="es-content" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p40t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0r es-m-p0l">
																									<p>Si tienes alguna pregunta o sugerencia, no dudes en ponerte en contacto con nosotros. Estamos aquí para ayudarte en todo lo que necesites.<br></p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<!--[if mso]><table width="500" cellpadding="0" cellspacing="0"><tr><td width="240" valign="top"><![endif]-->
																	<table cellpadding="0" cellspacing="0" class="es-left" align="left">
																		<tbody>
																			<tr>
																				<td width="240" class="es-m-p20b esd-container-frame" align="left">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-button">
																									<!--[if mso]><a href="https://front-programa-red.vercel.app/home" target="_blank" hidden>
					<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" esdevVmlButton href="https://front-programa-red.vercel.app/home" 
								style="height:48px; v-text-anchor:middle; width:117px" arcsize="0%" stroke="f"  fillcolor="#538d22">
						<w:anchorlock></w:anchorlock>
						<center style='color:#ffffff; font-family:Sora, Arial, sans-serif; font-size:19px; font-weight:700; line-height:19px;  mso-text-raise:1px'>HOME</center>
					</v:roundrect></a>
				<![endif]-->
																									<!--[if !mso]><!-- --><span class="msohide es-button-border"><a href="https://front-programa-red.vercel.app/home" class="es-button" target="_blank" style="font-weight: bold; font-size: 24px;">HOME</a></span>
																									<!--<![endif]-->
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																	<!--[if mso]></td><td width="20"></td><td width="240" valign="top"><![endif]-->
																	<table cellpadding="0" cellspacing="0" class="es-right" align="right">
																		<tbody>
																			<tr>
																				<td width="240" align="left" class="esd-container-frame">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-button">
																									<!--[if mso]><a href="https://front-programa-red.vercel.app/about" target="_blank" hidden>
					<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" esdevVmlButton href="https://front-programa-red.vercel.app/about" 
								style="height:48px; v-text-anchor:middle; width:129px" arcsize="0%" stroke="f"  fillcolor="#538d22">
						<w:anchorlock></w:anchorlock>
						<center style='color:#ffffff; font-family:Sora, Arial, sans-serif; font-size:19px; font-weight:700; line-height:19px;  mso-text-raise:1px'>ABOUT</center>
					</v:roundrect></a>
				<![endif]-->
																									<!--[if !mso]><!-- --><span class="msohide es-button-border"><a href="https://front-programa-red.vercel.app/about" class="es-button" target="_blank" style="font-size: 24px; font-weight: bold;">ABOUT</a></span>
																									<!--<![endif]-->
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																	<!--[if mso]></td></tr></table><![endif]-->
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="esd-footer-popover es-footer" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-footer-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text">
																									<p><u>podes Comunicate con Nostros</u></p>
																									<p>programared2023@gmail.com</p>
																									<p>o contactar con nosotros en&nbsp;nuestro gitHub</p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://github.com/programared2023"><img class="adapt-img" src="https://icones.pro/wp-content/uploads/2021/06/icone-github-verte.png" alt style="display: block;" width="80"></a></td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40b es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="left">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-spacer es-p30t es-p30b es-p40r es-p40l es-m-p0r es-m-p0l" style="font-size:0">
																									<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
																										<tbody>
																											<tr>
																												<td style="border-bottom: 2px solid #538d22; background: unset; height: 1px; width: 100%; margin: 0px;"></td>
																											</tr>
																										</tbody>
																									</table>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
								</td>
							</tr>
						</tbody>
					</table>
				</div>
			</body>
			
			`
		return bienvenida
	},
	mailDesbaneo: function (username) {
		let bienvenida = `
			<body>
				<div class="es-wrapper-color">
					<!--[if gte mso 9]>
						<v:background xmlns:v="urn:schemas-microsoft-com:vml" fill="t">
							<v:fill type="tile" color="#aad576"></v:fill>
						</v:background>
					<![endif]-->
					<table class="es-wrapper" width="100%" cellspacing="0" cellpadding="0">
						<tbody>
							<tr>
								<td class="esd-email-paddings" valign="top">
									<table cellpadding="0" cellspacing="0" class="esd-header-popover es-header" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-header-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure esdev-adapt-off" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="580" class="es-m-p0r esd-container-frame" valign="top" align="center">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://front-programa-red.vercel.app/home"><img class="adapt-img" src="https://gtfqov.stripocdn.email/content/guids/CABINET_bc226981bf9a9de9b3e1f896df4c90639fbca5d662d15ccbfabf150976647ae8/images/logonombre.png" alt style="display: block;" width="205"></a></td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="es-content" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p40" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text es-p10b es-p40r es-p40l es-m-p0r es-m-p0l">
																									<h1 style="font-size: 32px; text-align: center;">¡Fuiste desbaneado de <span style="font-size:34px;">ProgramaRed&nbsp; ${username}🤗!</span>!&nbsp;</h1>
																								</td>
																							</tr>
																							<tr>
																								<td align="left" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0">
																									<p style="font-size: 16px;">👏Nos complace darte la bienvenida de nuevo a nuestra Red Social 🙌, no olvides respetar las reglas del sitio ya que podrias ser ⚠baneado de por vida ⚠</p>
																									<p style="font-size: 16px;"><br></p>
																									<p>👮‍♀️ Los administradores siempre estamos observando 👮‍♂️...</p>
																									<p><br></p>
																									<p style="font-size: 32px; text-align: center;">¡Bienvenido de nuevo a ProgramaRed!🚀</p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="es-content" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p40t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0r es-m-p0l">
																									<p>Si tienes alguna pregunta o sugerencia, no dudes en ponerte en contacto con nosotros. Estamos aquí para ayudarte en todo lo que necesites.<br></p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<!--[if mso]><table width="500" cellpadding="0" cellspacing="0"><tr><td width="240" valign="top"><![endif]-->
																	<table cellpadding="0" cellspacing="0" class="es-left" align="left">
																		<tbody>
																			<tr>
																				<td width="240" class="es-m-p20b esd-container-frame" align="left">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-button">
																									<!--[if mso]><a href="https://front-programa-red.vercel.app/home" target="_blank" hidden>
					<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" esdevVmlButton href="https://front-programa-red.vercel.app/home" 
								style="height:48px; v-text-anchor:middle; width:117px" arcsize="0%" stroke="f"  fillcolor="#538d22">
						<w:anchorlock></w:anchorlock>
						<center style='color:#ffffff; font-family:Sora, Arial, sans-serif; font-size:19px; font-weight:700; line-height:19px;  mso-text-raise:1px'>HOME</center>
					</v:roundrect></a>
				<![endif]-->
																									<!--[if !mso]><!-- --><span class="msohide es-button-border"><a href="https://front-programa-red.vercel.app/home" class="es-button" target="_blank" style="font-weight: bold; font-size: 24px;">HOME</a></span>
																									<!--<![endif]-->
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																	<!--[if mso]></td><td width="20"></td><td width="240" valign="top"><![endif]-->
																	<table cellpadding="0" cellspacing="0" class="es-right" align="right">
																		<tbody>
																			<tr>
																				<td width="240" align="left" class="esd-container-frame">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-button">
																									<!--[if mso]><a href="https://front-programa-red.vercel.app/about" target="_blank" hidden>
					<v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" esdevVmlButton href="https://front-programa-red.vercel.app/about" 
								style="height:48px; v-text-anchor:middle; width:129px" arcsize="0%" stroke="f"  fillcolor="#538d22">
						<w:anchorlock></w:anchorlock>
						<center style='color:#ffffff; font-family:Sora, Arial, sans-serif; font-size:19px; font-weight:700; line-height:19px;  mso-text-raise:1px'>ABOUT</center>
					</v:roundrect></a>
				<![endif]-->
																									<!--[if !mso]><!-- --><span class="msohide es-button-border"><a href="https://front-programa-red.vercel.app/about" class="es-button" target="_blank" style="font-size: 24px; font-weight: bold;">ABOUT</a></span>
																									<!--<![endif]-->
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																	<!--[if mso]></td></tr></table><![endif]-->
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="esd-footer-popover es-footer" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-footer-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text">
																									<p><u>podes Comunicate con Nostros</u></p>
																									<p>programared2023@gmail.com</p>
																									<p>o contactar con nosotros en&nbsp;nuestro gitHub</p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://github.com/programared2023"><img class="adapt-img" src="https://icones.pro/wp-content/uploads/2021/06/icone-github-verte.png" alt style="display: block;" width="80"></a></td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40b es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="left">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-spacer es-p30t es-p30b es-p40r es-p40l es-m-p0r es-m-p0l" style="font-size:0">
																									<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
																										<tbody>
																											<tr>
																												<td style="border-bottom: 2px solid #538d22; background: unset; height: 1px; width: 100%; margin: 0px;"></td>
																											</tr>
																										</tbody>
																									</table>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
								</td>
							</tr>
						</tbody>
					</table>
				</div>
			</body>
			
			`
		return bienvenida
	},
	mailBaneo: function (username) {
		let bienvenida = `
			<body>
				<div class="es-wrapper-color">
					<!--[if gte mso 9]>
						<v:background xmlns:v="urn:schemas-microsoft-com:vml" fill="t">
							<v:fill type="tile" color="#aad576"></v:fill>
						</v:background>
					<![endif]-->
					<table class="es-wrapper" width="100%" cellspacing="0" cellpadding="0">
						<tbody>
							<tr>
								<td class="esd-email-paddings" valign="top">
									<table cellpadding="0" cellspacing="0" class="esd-header-popover es-header" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-header-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure esdev-adapt-off" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="580" class="es-m-p0r esd-container-frame" valign="top" align="center">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://front-programa-red.vercel.app/home"><img class="adapt-img" src="https://gtfqov.stripocdn.email/content/guids/CABINET_bc226981bf9a9de9b3e1f896df4c90639fbca5d662d15ccbfabf150976647ae8/images/logonombre.png" alt style="display: block;" width="205"></a></td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="es-content" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p40" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text es-p10b es-p40r es-p40l es-m-p0r es-m-p0l">
																									<h1 style="font-size: 32px; text-align: center;">¡Fuiste baneado de <span style="font-size:34px;">ProgramaRed&nbsp; ${username} 😱</span>!&nbsp;</h1>
																								</td>
																							</tr>
																							<tr>
																								<td align="left" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0">
																									<p style="font-size: 16px;"> ⚠ Recibiste este correo porque tu cuenta fue baneada de nuestro sitio, por favor comunicate con algun administrador del mismo para apelar a este baneo. ⚠ </p>
																									
<p><br></p>
																						
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="es-content" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-content-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p40t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text es-p20t es-p20b es-p40r es-p40l es-m-p0r es-m-p0l">
																									<p>Si consideras que este baneo fue un error, comunicate con los admins del sitio para apelar tu queja 🤔.<br></p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<!--[if mso]><table width="500" cellpadding="0" cellspacing="0"><tr><td width="240" valign="top"><![endif]-->
																	<table cellpadding="0" cellspacing="0" class="es-left" align="left">
																		<tbody>
																			<tr>
																				<td width="240" class="es-m-p20b esd-container-frame" align="left">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-button">
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																	<!--[if mso]></td><td width="20"></td><td width="240" valign="top"><![endif]-->
																	<table cellpadding="0" cellspacing="0" class="es-right" align="right">
																		<tbody>
																			<tr>
																				<td width="240" align="left" class="esd-container-frame">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-button">
																									
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																	<!--[if mso]></td></tr></table><![endif]-->
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
									<table cellpadding="0" cellspacing="0" class="esd-footer-popover es-footer" align="center">
										<tbody>
											<tr>
												<td class="esd-stripe" align="center">
													<table bgcolor="#ffffff" class="es-footer-body" align="center" cellpadding="0" cellspacing="0" width="580">
														<tbody>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-text">
																									<p><u>podes Comunicate con Nostros</u></p>
																									<p>programared2023@gmail.com</p>
																									<p>o contactar con nosotros en&nbsp;nuestro gitHub</p>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="center" valign="top">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-image" style="font-size: 0px;"><a target="_blank" href="https://github.com/programared2023"><img class="adapt-img" src="https://icones.pro/wp-content/uploads/2021/06/icone-github-verte.png" alt style="display: block;" width="80"></a></td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
															<tr>
																<td class="esd-structure es-p20t es-p40b es-p40r es-p40l" align="left">
																	<table cellpadding="0" cellspacing="0" width="100%">
																		<tbody>
																			<tr>
																				<td width="500" class="esd-container-frame" align="left">
																					<table cellpadding="0" cellspacing="0" width="100%">
																						<tbody>
																							<tr>
																								<td align="center" class="esd-block-spacer es-p30t es-p30b es-p40r es-p40l es-m-p0r es-m-p0l" style="font-size:0">
																									<table border="0" width="100%" height="100%" cellpadding="0" cellspacing="0">
																										<tbody>
																											<tr>
																												<td style="border-bottom: 2px solid #538d22; background: unset; height: 1px; width: 100%; margin: 0px;"></td>
																											</tr>
																										</tbody>
																									</table>
																								</td>
																							</tr>
																						</tbody>
																					</table>
																				</td>
																			</tr>
																		</tbody>
																	</table>
																</td>
															</tr>
														</tbody>
													</table>
												</td>
											</tr>
										</tbody>
									</table>
								</td>
							</tr>
						</tbody>
					</table>
				</div>
			</body>
			
			`
		return bienvenida
	}
}